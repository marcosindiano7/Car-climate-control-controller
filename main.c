...
void OS_Pin_Init (void){
  ...
  GPIO_InitTypeDef g = {
    .Pin   = LM75_OS_PIN,
    .Mode  = GPIO_MODE_IT_RISING_FALLING, /* dos flancos */
    .Pull  = GPIO_PULLUP                  /* OS es ‘open-collector’ */
  };
  __HAL_RCC_GPIOC_CLK_ENABLE();
  HAL_GPIO_Init(LM75_OS_GPIO_PORT, &g);
  HAL_NVIC_SetPriority(LM75_OS_IRQn, 5, 0);
  HAL_NVIC_EnableIRQ(LM75_OS_IRQn);
}
