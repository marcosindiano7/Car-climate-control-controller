...
extern osMessageQueueId_t   m_MQID_USART_SEND;     // de com.c transmisión 
extern osMessageQueueId_t   m_MQID_USART_RECEIVE;  // de com.c recepción
...
static void procesar_pot (void)
{
  ...
  if (status == osOK) {
    Te = pot.Te;
    Ta = pot.Ta;
		MSGQUEUE_SENS_TA_t Ta_env;
		Ta_env.Ta = Ta;
		osMessageQueuePut(sens_TaQueue, &Ta_env, 0, 0);
  }
}
...
{
  if (GPIO_Pin & (UP | RIGHT | DOWN | LEFT | CENTER)) {
    osThreadFlagsSet(tid_ThJOYSTICK, PULSACION);
    return;
  }

  if (GPIO_Pin == LM75_OS_PIN && (modo == MODO_ACTIVO || modo == MODO_TEST)) {
    if (HAL_GPIO_ReadPin(LM75_OS_GPIO_PORT, LM75_OS_PIN) == GPIO_PIN_RESET) {
      if (!alarma_activa) {
        alarma_activa = true;
...
static void enviar_trama(uint8_t cmd, const char *payload)
{
  ...
  msg.data[1]  = cmd;
  msg.data[2]  = len;
  strncpy((char *)&msg.data[3], payload, len - 4U);
  msg.data[len - 1U] = EOT;

  osMessageQueuePut(m_MQID_USART_SEND, &msg, 0U, 0U);
}

static void procesar_comandos(void)
{
  G_DATA_FROM_USART frame;
  osStatus_t        status;

  status = osMessageQueueGet(m_MQID_USART_RECEIVE, &frame, NULL, 0U);
  if (status != osOK) {
    return;
  }
  ...
  uint8_t payload_len = (frame.length > 4U) ? (frame.length - 4U) : 0U;
  memcpy(payload_raw, &frame.data[3], payload_len);
